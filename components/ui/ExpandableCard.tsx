...
export function ExpandableProjectCards({ limit }: { limit?: number }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {activeProject && (
          <div className={`fixed ${isFullscreen ? 'inset-0' : 'inset-0 flex items-start justify-center p-4 sm:p-6'} z-[60] overflow-y-auto`}>
            <motion.div
              layoutId={`card-${activeProject.id}-${id}`}
              ref={ref}
              className={`${isFullscreen ? 'w-full h-full rounded-none' : 'w-full max-w-5xl rounded-2xl max-h-[90vh]'} bg-white dark:bg-gray-800 shadow-2xl overflow-hidden flex flex-col`}
            ...
            >
              <div className="relative">
                <img
                  ...
                  className={`w-full ${isFullscreen ? 'h-48 sm:h-56' : 'h-48 sm:h-72'} object-cover`}
                />
                    ...
                    <div className="bg-gray-50 dark:bg-gray-700 p-4 sm:p-6 rounded-lg">
                      ...
                      <ul className="space-y-2 sm:space-y-3">
                        {activeProject.details.features.map((feature, index) => (
                          ...
                          >
                            <span className="text-[#D1F2EB] mt-1 text-lg sm:text-xl">•</span>
                            <span>{feature}</span>
                          </motion.li>
                        ))}
                      </ul>
            ...
            <div className="p-4 sm:p-6">
              <div className="flex flex-wrap gap-2 sm:gap-3 mb-3 sm:mb-4">
                ...
                {project.techStack.length > 3 && (
                  <span className="text-[0.7rem] sm:text-[0.78rem] bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-[#D1F2EB] px-2 py-1 sm:px-3 sm:py-1.5 rounded-full font-medium">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 justify-start">
                ...
                <button className="text-xs sm:text-sm ml-auto bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-800 dark:text-[#D1F2EB] px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg font-medium">
                  View Details
                </button>
              </div>
            </div>
